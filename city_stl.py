...
Options:
    ...
    --no-preview        skip the PNG previews (one per tile + assembled view)
    --scout             don't build; write NAME_landmark_suggestions.json/.txt listing statues,
                        monuments, towers, cathedrals, stadiums and the tallest buildings in
                        the area, pre-filled for landmarks.json
...
SCOUT_TAGS = {
    ...
    "historic": True,
    "man_made": ["tower", "lighthouse", "obelisk", "monument", "communications_tower", "bridge"],
    "amenity": ["place_of_worship"],
    "leisure": ["stadium"],
    "building": ["cathedral", "church", "temple", "mosque", "synagogue", "stadium", "castle",
                 "palace", "government", "civic", "train_station", "observatory"],
    "memorial": True,
    "wikidata": True,
}
_SCORE = [("memorial", 5), ("historic", 4), ("tourism", 4), ("man_made", 3), ("wikidata", 3),
          ("wikipedia", 2), ("amenity", 1), ("leisure", 2), ("building", 1)]


def scout_landmarks(bbox_wgs, epsg, clip_box, to_wgs, top_n=25, tallest_n=8):
    """Find things in the area that won't look right as extruded boxes and should get
    their own STL: statues, monuments, towers, cathedrals, stadiums, and the tallest
    buildings. Returns a list of ready-to-fill landmarks.json entries."""
    west, south, east, north = bbox_wgs
    print("  scouting for notable features...")
    try:
        gdf = ox.features_from_bbox((west, south, east, north), tags=SCOUT_TAGS)
    except Exception as e:
        print(f"  scouting failed ({e})")
        gdf = None
    cands = {}

    def add(name, geom_m, tags, score, kind):
        if not name or geom_m.is_empty:
            return
        c = geom_m.centroid
        if not clip_box.contains(c):
            return
        key = name.lower()
        h_new = _num(tags.get("height")) or _num(tags.get("building:height"))
        if key in cands and cands[key]["_score"] >= score:
            if cands[key]["height_m"] == "FILL_IN" and h_new:     # keep entry, borrow the height
                cands[key]["height_m"] = round(h_new)
            return
        lon, lat = to_wgs.transform(c.x, c.y)
        area = geom_m.area if geom_m.geom_type != "Point" else 0.0
        radius = max(40.0, math.sqrt(area / math.pi) * 1.6) if area else 40.0
        h = _num(tags.get("height")) or _num(tags.get("building:height"))
        if h is None:
            lv = _num(tags.get("building:levels"))
            h = lv * 3.2 + 1 if lv else None
        if h is None and key in cands and cands[key]["height_m"] != "FILL_IN":
            h = cands[key]["height_m"]
        entry = {"name": name, "lat": round(lat, 6), "lon": round(lon, 6),
                 "file": re.sub(r"[^a-z0-9]+", "_", name.lower()).strip("_") + ".stl",
                 "height_m": round(h) if h else "FILL_IN",
                 "radius_m": round(radius), "match_name": name,
                 "_why": kind, "_score": score}
        cands[key] = entry
    ...
    if gdf is not None and len(gdf):
        ...
        for _, row in gdf.iterrows():
            tags = {c: row[c] for c in cols if isinstance(row[c], str)}
            name = tags.get("name") or tags.get("name:en")
            if not name:
                continue
            score = sum(w for k, w in _SCORE if k in tags)
            why = "; ".join(f"{k}={tags[k]}" for k in ("memorial", "historic", "tourism", "man_made",
                                                        "amenity", "leisure", "building") if k in tags)
            add(name, row.geometry, tags, score, why or "notable")

    # tallest buildings — with a stated height and a name
    try:
        b = ox.features_from_bbox((west, south, east, north), tags={"building": True}).to_crs(epsg=epsg)
        if "name" in b.columns:
            b = b[b["name"].notna()]
            b = b.assign(_h=[_num(v) for v in (b["height"] if "height" in b.columns else [None] * len(b))])
            b = b[b["_h"].notna()].sort_values("_h", ascending=False).head(tallest_n)
            for _, row in b.iterrows():
                tags = {c: row[c] for c in b.columns if c != "geometry" and isinstance(row[c], str)}
                add(str(row["name"]), row.geometry, tags, 2 + row["_h"] / 100,
                    f"one of the tallest here ({row['_h']:.0f} m)")
    except Exception as e:
        print(f"  (tallest-building scan skipped: {e})")

    ranked = sorted(cands.values(), key=lambda d: -d["_score"])[:top_n]
    return ranked


def write_scout_report(entries, stem, landmarks_path):
    have = {os.path.basename(i["file"]) for i in load_landmarks(landmarks_path)}
    out_json = f"{stem}_landmark_suggestions.json"
    out_txt = f"{stem}_landmark_suggestions.txt"
    clean = [{k: v for k, v in e.items() if not k.startswith("_")} for e in entries]
    with open(out_json, "w") as fh:
        json.dump(clean, fh, indent=2)
    lines = ["Suggested landmarks — things that won't look right as plain extruded boxes.",
             "Add the ones you care about to landmarks/landmarks.json and upload a matching STL.",
             ""]
    for e in entries:
        flag = "  [model present]" if e["file"] in have else ""
        h = f"{e['height_m']} m" if e["height_m"] != "FILL_IN" else "height unknown — fill in"
        lines.append(f"- {e['name']}  ({e['lat']}, {e['lon']})  {h}  — {e['_why']}{flag}")
        lines.append(f"    file: landmarks/{e['file']}   radius_m: {e['radius_m']}")
    txt = "\n".join(lines)
...
# ----------------------------------------------------------------------------- previews
def render_preview(meshes_with_offsets, out_png, az_deg=225, el_deg=35, title=None,
                   max_faces=3_000_000, seams=None):
    """Quick shaded isometric render (painter's algorithm) — no OpenGL needed."""
    import matplotlib
    matplotlib.use("Agg")
    ...
    V, F, b = [], [], 0
    for m, (dx, dy) in meshes_with_offsets:
        V.append(m.vertices + [dx, dy, 0])
        F.append(m.faces + b)
        b += len(m.vertices)
    V, F = np.vstack(V), np.vstack(F)
    if len(F) > max_faces:                       # keep the biggest faces
        area = np.linalg.norm(np.cross(V[F[:, 1]] - V[F[:, 0]], V[F[:, 2]] - V[F[:, 0]]), axis=1)
        F = F[np.argsort(-area)[:max_faces]]
    a, e = math.radians(az_deg), math.radians(el_deg)
    R = np.array([[math.cos(a), -math.sin(a), 0], [math.sin(a), math.cos(a), 0], [0, 0, 1]])
    P = V @ R.T
    x = P[:, 0]
    y = P[:, 1] * math.sin(e) + P[:, 2] * math.cos(e)
    depth = P[:, 1] * math.cos(e) - P[:, 2] * math.sin(e)
    n = np.cross(V[F[:, 1]] - V[F[:, 0]], V[F[:, 2]] - V[F[:, 0]])
    nn = np.linalg.norm(n, axis=1)
    ok = nn > 1e-12
    F, n = F[ok], n[ok] / nn[ok, None]
    key = np.array([-0.4, -0.6, 0.7]); key /= np.linalg.norm(key)
    fill = np.array([0.7, 0.3, 0.2]); fill /= np.linalg.norm(fill)
    shade = 0.2 + 0.6 * np.clip(n @ key, 0, 1) + 0.25 * np.clip(n @ fill, 0, 1)
    order = np.argsort(-depth[F].mean(1))
    polys = np.stack([x[F], y[F]], -1)[order]
    col = np.clip(shade[order][:, None] * np.array([0.90, 0.91, 0.93]), 0, 1)
    fig = plt.figure(figsize=(9, 9))
    ax = fig.add_axes([0.02, 0.02, 0.96, 0.92])
    ax.set_aspect("equal"); ax.axis("off")
    ax.add_collection(PolyCollection(polys, facecolors=col, edgecolors="none", antialiased=False))
    if seams:                                    # dashed lines where tiles meet
        segs = []
        for (x0, y0, x1, y1) in seams:
            q = np.array([[x0, y0, 0], [x1, y1, 0]]) @ R.T
            segs.append(np.column_stack([q[:, 0], q[:, 1] * math.sin(e) + q[:, 2] * math.cos(e)]))
        ax.add_collection(LineCollection(segs, colors="#d04030", linewidths=1.2, linestyles="--"))
...
def main():
    ...
    ap.add_argument("--no-preview", action="store_true", help="skip PNG renders")
    ap.add_argument("--scout", action="store_true",
                    help="don't build; list landmarks in the area that deserve their own STL")
    ...
    if a.scout:
        entries = scout_landmarks((west, south, east, north), epsg,
                                  box(xmin, ymin, xmax, ymax), to_wgs)
        write_scout_report(entries, os.path.splitext(a.output)[0], a.landmarks)
        return 0
    ...
    for r in range(rows):
        for c_ in range(cols):
            ...
            written.append(name)
            if not a.no_preview:
                png = os.path.splitext(name)[0] + "_preview.png"
                render_preview([(mesh, (0, 0))], png,
                               title=f"{os.path.basename(name)}  —  {e[0]:.0f} x {e[1]:.0f} x {e[2]:.1f} mm")
                tile_meshes.append((mesh, ((tx0 - xmin) * scale, (ty0 - ymin) * scale)))
    if not a.no_preview and cols * rows > 1:
        seams = [((xs[i] - xmin) * scale, 0, (xs[i] - xmin) * scale, (ymax - ymin) * scale) for i in xi[1:-1]] + \
                [(0, (ys[j] - ymin) * scale, (xmax - xmin) * scale, (ys[j] - ymin) * scale) for j in yi[1:-1]]
        render_preview(tile_meshes, f"{stem}_assembled.png", seams=seams,
                       title=f"Assembled {cols}x{rows} tiles — {a.width_mm:.0f} x {a.width_mm * a.y_km / a.x_km:.0f} mm")
        print(f"Wrote {stem}_assembled.png")
    if cols * rows > 1:
